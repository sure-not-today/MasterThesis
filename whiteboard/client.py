...
async def main():
    ...
    global status
    status = Status(Position(0,0,0), True, 0, 200, LandedState(0))
    ...
    while True:
        if status.idle:
            msg = f"TASK_{status.pos.lat}_{status.pos.long}_{status.pos.alt}"
        ...
        if message.startswith("EMPTY"):
            ...
            space = max(status.capacity - status.payload, 0)
            msg = f"REACHED_{pos.lat}_{pos.long}_{pos.alt}_{space}"
            writer.write(msg.encode())
            await writer.drain()
